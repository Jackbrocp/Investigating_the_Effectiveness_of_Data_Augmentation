...
def load_torchvision_data(dataname, valid_size=0.1, splits=None, shuffle=True,seed =0,
                    stratified=False, random_seed=None, batch_size = 256,
...
def load_imagenet(datadir=None, transform=None, train=True ,resize=None, tiny=False, augmentations=False, **kwargs):
    ...
    fold_loaders, dsets = load_torchvision_data('Imagenet', transform=transform,
                                                data=(train_data, valid_data),
                                                **kwargs)
    if train:
        return fold_loaders['train']
    else:
        return fold_loaders['test']
