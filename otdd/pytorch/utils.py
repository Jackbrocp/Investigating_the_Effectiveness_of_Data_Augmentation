...
import sys 
sys.path.append('../..')
from data_augmentation import keepaugment,mixup,cutmix
...
def load_full_dataset(data, targets=False, return_both_targets=False,
    ...
    Arguments:
        ...
        augmentation (bool): use some kind of augmentation which can not be in the transform,
                             including keepaugment, mixup, cutmix
        ...
        if maxsamples:
            if hasattr(loader, 'sampler') and hasattr(loader.sampler, 'indices'):
                if len(loader.sampler.indices) <= maxsamples:
                    print('Indices:Maxsamples is greater than number of effective examples in loader. Will not subsample.')
                else:
                ...
                if len(loader.dataset) <= maxsamples:
                    print('dataset:Maxsamples is greater than number of examples in loader. Will not subsample.')
                else:
                    ## Create new sampler
    ...
    # if augmentation == 'keepaugment':
    #     aug = keepaugment.Keep_Autoaugment(dataset='CIFAR10',length=16,N=3, M=24)
    ...
    for batch in tqdm(loader, leave=False):
        ...
        if augmentation == 'mixup':
            x,y = mixup.mixup_operation(x,y)
        elif augmentation == 'cutmix':
            x,y = cutmix.cutmix_operation(x,y)
        elif augmentation == 'keepaugment':
            keep = keepaugment.Keep_Autoaugment(dataset='CIFAR10',length=16,N=3, M=24)
            x = keep(x)
