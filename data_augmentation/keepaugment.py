...
class Keep_Autoaugment(object):
    def __init__(self,  dataset,  length, N, M, early=False):
        self.length = length
        self.n = N
        self.m = M
        self.early = early
        self.denomal = transforms.Normalize(*DATASET_NORMALIZATION[dataset])
        self.tensor = transforms.ToTensor()
        self.pil = transforms.ToPILImage()
        self.auto = CIFAR10Policy()
        self.trans = train_transform = transforms.Compose([
                transforms.RandomCrop(32, padding=4),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                transforms.Normalize(*DATASET_NORMALIZATION[dataset])])
        self.model = ResNet18().eval()
        for p in self.model.parameters():
            p.requires_grad = False
        self.model = torch.nn.DataParallel(self.model, device_ids=[0,1]).cuda()
        # model_path = '/home/yangsuorong/Optimal_transport/otdd-main/embedder/resnet18_best.pth'
        model_path = '/home/yangsuorong/Optimal_transport/otdd-main/CIFAR100/embedder/embedder_CIFAR100_resnet18.pth'
    ...
    def __call__(self, images):
        ...
        if self.early:
            preds = self.model(images_,True)
        else:
            preds = self.model(images_)
...
if __name__ == '__main__':
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize([x / 255.0 for x in [125.3, 123.0, 113.9]], [x / 255.0 for x in [63.0, 62.1, 66.7]])])
    mean = [x / 255.0 for x in [125.3, 123.0, 113.9]]
    std = [x / 255.0 for x in [63.0, 62.1, 66.7]]
    ka = Keep_Autoaugment(train_transform,mean,std,16,3,24)
